...
use Config\Services;
use App\Models\Modeldatalowongan;
...
class Lowongan extends BaseController
{
	...
	{
		$data = [
			'title' => 'SISTEM INFORMASI PENERIMAAN PEGAWAI BARU | DATA LOWONGAN',
			'page' => 'LOWONGAN'
		];
		return view('Page/Lowongan/Viewdata', $data);
	}
	public function ambildata()
	{
		if ($this->request->isAJAX()) {
			...
			$msg = [
				'data' => view('Page/Lowongan/Data', $data)
			];
	...
	public function listdata()
	{
		...
		$datamodel = new Modeldatalowongan($request);
		if ($request->getMethod(true) == 'POST') {
			...
			foreach ($lists as $list) {
				...
				$no++;
				$row = [];
				$tomboledit = "<button type=\"button\" title=\"Edit \"  class=\"btn btn-primary btn-sm\" onclick=\"edit('" . $list->id_lowongan .
					"')\"><i class=\"mdi mdi-transcribe\"></i></button>";
				$tombolhapus = " <button type=\"button\" title=\"Hapus \"  class=\"btn btn-danger btn-sm\" onclick=\"hapus('" . $list->id_lowongan .
	...
	public function formstatus()
	{
		if ($this->request->isAJAX()) {
			...
			$data = [
				'id_lowongan' => $row['id_lowongan'],
				'status' => $row['status_lowongan'],
			];

			$msg = [
				'sukses' => view('Page/Lowongan/modalstatus', $data)
			];
			echo json_encode($msg);
		}
	}
	public function updatestatus()
	{
		if ($this->request->isAJAX()) {
			...
			if (!$valid) {
				...
				$this->Lowongan->update($id, $simpandata);
				$msg = [
	...
	public function formedit()
	{
		if ($this->request->isAJAX()) {
			...
			$msg = [
				'sukses' => view('Page/Lowongan/modaledit', $data)
			];
			echo json_encode($msg);
		}
	}
	...
	public function updatedata()
	{
		...
		if ($this->request->isAJAX()) {
			...
			if (!$valid) {
				...
				$this->Lowongan->update($id, $simpandata);
				$msg = [
					'sukses' => 'Data Lowongan Berhasil di Edit'
				];
			}
			echo json_encode($msg);
		} else {
