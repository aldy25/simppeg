...
use Config\Services;
use App\Models\Modaldataseleksi;
...
class Seleksi extends BaseController
{
    ...
    {
        $data = [
            'title' => 'SISTEM INFORMASI PENERIMAAN PEGAWAI BARU | SELEKSI PELAMAR',
            'page' => 'SELEKSI PELAMAR'
        ];
        return view('Page/Seleksi/Viewdata', $data);
    }
    public function ambildata()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'data' => view('Page/Seleksi/Data', $data)
            ];
    ...
    public function listdata()
    {
        ...
        $datamodel = new Modaldataseleksi($request);
        if ($request->getMethod(true) == 'POST') {
            ...
            foreach ($lists as $list) {
                ...
                $status = $list->status;
                if ($status == 'Terkirim') {
                    $tombolstatus = "<button type=\"button\"   class=\"btn btn-warning  btn-sm\"  onclick=\"Status('" . $list->id_lamaran .
                        "')\">$status</button>";
                } elseif ($status == 'Diproses') {
                    $tombolstatus = "<button type=\"button\"   class=\"btn  btn-warning btn-sm\" onclick=\"Status('" . $list->id_lamaran .
                        "')\">$status</button>";
                } elseif ($status == 'Ditolak') {
                    $tombolstatus = "<button type=\"button\"   class=\"btn btn-danger btn-sm\" onclick=\"Status('" . $list->id_lamaran .
                        "')\">$status</button>";
                } else {
                    $tombolstatus = "<button type=\"button\"   class=\"btn btn-success btn-sm\" onclick=\"Status('" . $list->id_lamaran .
                        "')\">$status</button>";
                }
                $row[] = $no;
    ...
    public function formstatus()
    {
        if ($this->request->isAJAX()) {
            ...
            $msg = [
                'sukses' => view('Page/Seleksi/modalstatus', $data)
            ];
            echo json_encode($msg);
        }
    }
    public function updatestatus()
    {
        if ($this->request->isAJAX()) {
            ...
            if (!$valid) {
                ...
                $this->Riwayat->update($id, $simpandata);
                $msg = [
